  ...
  if(!is.null(phase)){
    if(phase == 'baseline'){
      ...
      graph <- ggpubr::ggbarplot(
        ...
        add = 'mean', palette = c('blue', 'black'), fill = 'GroupBehavior', ylim=c(0.05,0.8), xlab = xlab, ylab = ylab,
        position = ggplot2::position_dodge(0.8), ggtheme = ggplot2::theme_bw(), width = 0.6
      )
    } else {
