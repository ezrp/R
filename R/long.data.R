...
long.data <- function(data, phase = NULL, within = NULL, quest = NULL, score = NULL){
  ...
  if(!is.null(phase)){
    if(phase == 'baseline'){
      ...
      df <- dplyr::transmute(data, id = id, induction = induction, Equitable = Eq_B6, NonEquitable = NonEq_B6)

      long <- df %>%
        rstatix::gather(key = 'GroupBehavior', value = 'Cooperation', Equitable, NonEquitable) %>%
